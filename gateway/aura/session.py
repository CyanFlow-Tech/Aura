...
class SessionError(Exception):
    """Base error for session-layer failures."""
...
class NoActiveTurnError(SessionError):
    """Raised when a session has no active turn to stream from."""
...
class SessionManager(LoggingMixin):
    ...
    def get_session(self, session_id: str) -> Session:
        ...
        if session is None:
            raise SessionNotFoundError(
                "session_id not found or already released"
            )
        return session
    ...
    def subscribe(self, session: Session, channel_name: str) -> ReceiveChannel:
        ...
        if turn is None:
            raise NoActiveTurnError("session has no active turn; upload first")
        return turn.subscribe(channel_name)

    async def stream(self, session: Session, channel_name: str):
        """Iterate items from the current Turn's endpoint channel."""
        channel = self.subscribe(session, channel_name)
