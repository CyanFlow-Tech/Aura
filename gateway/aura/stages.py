...
class BaseConversationStage(LoggingMixin):
    """Shared skeleton for LLM-backed dialog stages.

    Subclasses only decide how to interpret their input and whether this
    turn needs extra system context. The streaming, batching, persistence,
    and output-close semantics stay in one place.
    """
    ...
    async def run(self) -> None:
        ...
        appended_user = False
        try:
            user_text, extra_system_messages = await self._prepare_turn()
            ...
            full_reply = await self._stream_reply(messages)
        finally:
    ...
    async def _prepare_turn(self) -> tuple[str, list[str]]:
        raise NotImplementedError("Subclasses must prepare the user turn")

    async def _stream_reply(self, messages: list[dict[str, str]]) -> str:
        full_reply = ""
        sentence = ""
...
class ConversationStage(BaseConversationStage):
    ...
    def __init__(
        ...
        super().__init__(llm, conversation, out)
        self._inp = inp

    async def _prepare_turn(self) -> tuple[str, list[str]]:
        return await self._inp.receive(), []
...
class SearchAugmentedConversationStage(BaseConversationStage):
    ...
    async def _prepare_turn(self) -> tuple[str, list[str]]:
        ...
        extra_system_messages = [retrieval_context] if retrieval_context else []
        return turn.user_text, extra_system_messages
