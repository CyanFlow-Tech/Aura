...
class QueueChannel(Generic[T]):
    ...
    def replay(self, items: list[T], closed: bool = False) -> None:
        """Seed a new consumer channel with buffered history.

        Intended for channel-composition helpers such as BroadcastChannel;
        keeps that code from reaching into QueueChannel internals.
        """
        for item in items:
...
class BroadcastChannel(Generic[T]):
    ...
    def subscribe(self) -> QueueChannel[T]:
        ...
        sub.replay(self._history, closed=self._closed)
        if not self._closed:
