...
from .session import NoActiveTurnError, SessionManager, SessionNotFoundError
...
def _subscribe_or_409(session_id: str):
    session = _get_session_or_404(session_id)
    try:
        return session_manager.subscribe(session, CHANNEL_TTS_OUT)
...
async def text_stream(session_id: str):
    ...
    channel = _subscribe_or_409(session_id)

    async def gen():
        async for sentence, _audio in channel:
